...
def ask_qwq_streaming(prompt, text_widget, tag="system", callback=None, player_id=None):
    """
    使用 Qwen/QwQ-32B 模型以流式方式生成内容，实时更新 text_widget  
      - 显示所有返回内容（包括思考过程）到 text_widget  
      - 仅将 "=== Final Answer ===" 之后的内容传递给回调函数（并存入日志文件），即不保存中间的思考过程
    """
    ...
    def stream_output():
        ...
        try:
            ...
            error_msg = f"\n[错误] QwQ 生成失败: {type(e).__name__}: {str(e)}\n"
