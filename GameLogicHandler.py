...
class GameLogicHandler:
    ...
    def apply_config(self):
        ...
        self.app.ui_handler.log_system("已应用新配置，重新初始化游戏。") # Changed to call ui_handler.log_system
    ...
    def restart_game(self):
        ...
        self.app.day_label.config(text=f"第 {self.app.state.day} 天")
        self.apply_config()
        self.app.ui_handler.log_system("游戏已重新开始。") # Changed to call ui_handler.log_system
    ...
    def update_player_identity(self, player_id, new_identity):
        ...
        if new_identity == "空":
            ...
            player.exists = True
        self.app.ui_handler.log_system(f"设置 玩家 {player_id} 的身份为 {new_identity}") # Changed to call ui_handler.log_system

    def update_player_model(self, player_id, new_model):
        player = self.app.state.players[player_id]
        player.model = new_model
        self.app.ui_handler.log_system(f"设置 玩家 {player_id} 的模型为 {new_model}") # Changed to call ui_handler.log_system
            ...
            if not vote_counts:
                ...
                if len(candidates) > 1:
                    ...
                    if sheriff_id in self.app.state.players and self.app.state.players[sheriff_id].alive:
                        ...
                        self.app.ui_handler.log_system(f"玩家 {sheriff_id} 被选为警长！") 
                        # 更新UI显示警长标签
        ...
        if not vote_counts:
            ...
            if len(candidates) > 1:
                ...
                if eliminated in self.app.state.players and self.app.state.players[eliminated].alive:
                    ...
                    self.app.ui_handler.log_system(f"玩家 {eliminated} 被白天投票淘汰！") # Changed to call ui_handler.log_system
                else:
                    self.app.ui_handler.log_system("投票结果错误：指定玩家不存在或已死亡。") # Changed to call ui_handler.log_system
    ...
    def start_night(self):
        self.app.ui_handler.log_system("【夜晚回合开始】") # Changed to call ui_handler.log_system
    ...
    def finalize_night_voting(self):
        ...
        self.app.ui_handler.log_system(f"【夜晚投票结算】狼人有效投票统计: {wolf_vote_counts}") # Changed to call ui_handler.log_system
        ...
        if wolf_vote_counts:
            ...
            if len(candidates) == 1:
                ...
                self.app.ui_handler.log_system(f"狼人决定击杀玩家 {wolf_target}") # Changed to call ui_handler.log_system
            else:
                self.app.ui_handler.log_system("狼人意见不统一，无法确定击杀目标") # Changed to call ui_handler.log_system
        else:
            self.app.ui_handler.log_system("狼人没有选择击杀目标") # Changed to call ui_handler.log_system
        ...
        if witch_acted:
            self.app.ui_handler.log_system(f"今晚行动的女巫: {witch_acted}") # Changed to call ui_handler.log_system
        ...
        if wolf_target and wolf_target != self.app.state.witch_save_target:
            if wolf_target in self.app.state.players and self.app.state.players[wolf_target].alive:
                ...
                self.app.ui_handler.log_system(f"玩家 {wolf_target} 被狼人击杀！") # Changed to call ui_handler.log_system
                
                # 启用遗言按钮
                self.app.lastword_buttons[wolf_target].config(state=tk.NORMAL)
        elif wolf_target == self.app.state.witch_save_target:
            self.app.ui_handler.log_system(f"玩家 {wolf_target} 被女巫救活了！") # Changed to call ui_handler.log_system
        
        # 夜晚投票结算的总结
        if not self.app.state.dead_today:
            self.app.ui_handler.log_system("夜晚平安夜：无人死亡。") # Changed to call ui_handler.log_system
    ...
    def next_round(self):
        self.app.ui_handler.log_system("【进入下一回合】") # Changed to call ui_handler.log_system
        
        # 如果当前是第0天（警长选举阶段），进入第1天正式开始游戏
        if self.app.state.day == 0:
            self.app.ui_handler.log_system("【警长选举阶段结束，进入第1天】") # Changed to call ui_handler.log_system
            self.next_round_init()
            self.app.print_day_status() # This calls log_system internally
            self.update_buttons_for_phase("day")
        else:
            # 正常进入下一天
            self.next_round_init()
    ...
    def print_day_status(self):
        ...
        self.app.ui_handler.log_system(status_text) # Changed to call ui_handler.log_system

    def correct_last_vote(self):
        last_voter_id = self.app.last_voter_id
        if not last_voter_id:
            self.app.ui_handler.log_system("[错误] 无法纠正投票： 未知最后投票玩家。") # Changed to call ui_handler.log_system
        ...
        if manual_vote_result:
            if self.app.state.phase == "day":
                ...
                log_type = "夜晚"
            self.app.ui_handler.log_system(f"[主持人手动纠正] 玩家 {last_voter_id} 的 {log_type}投票 被修改为: {manual_vote_result}") # Changed to call ui_handler.log_system
            messagebox.showinfo("投票已纠正", f"玩家 {last_voter_id} 的 {log_type}投票 已被手动纠正为: {manual_vote_result}")
        else:
            self.app.ui_handler.log_system(f"[提示] 主持人取消了 玩家 {last_voter_id} 的投票纠正操作。") # Changed to call ui_handler.log_system
