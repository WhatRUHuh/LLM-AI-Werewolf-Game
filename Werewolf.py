...
from SpeechHandler.SpeechHandler import SpeechHandler  # 导入 SpeechHandler 类
from VoteHandler.VoteHandler import VoteHandler  # 导入 VoteHandler 类
...
class StartPage:
    def __init__(self, root):
        ...
        self.bg_image_load_error = False  # 标记图片加载错误的标志
        try:
    ...
    def create_start_page(self):
        ...
        # 如果背景图片加载失败，则显示警告信息
        if self.bg_image_load_error:
            messagebox.showwarning("加载错误", "背景图片加载失败，将使用默认背景。")
...
class WerewolfGameApp:
    def __init__(self, root):
        ...
        # 初始化配置
        config_settings = init_config()
        self.tts_enabled = config_settings["tts_enabled"]
        http_proxy = config_settings["http_proxy"]
        https_proxy = config_settings["https_proxy"]
    ...
    def clear_summary_text(self):
        """清空游戏总统计窗口的文字"""
        self.ui_handler.app.summary_text.delete("1.0", tk.END)
        self.ui_handler.log_system("游戏统计信息已清空。")
    ...
    def apply_config(self):
        ...
        self.ui_handler.log_system(f"游戏配置已更新：玩家人数 = {self.player_count}，狼人数量 = {self.wolf_count}，预言家数量 = {self.seer_count}，猎人数量 = {self.hunter_count}，女巫数量 = {self.witch_count}")
    ...
    def toggle_sound(self):
        ...
        self.ui_handler.log_system(f"游戏音效已 {'启用' if self.sound_enabled else '禁用'}")
