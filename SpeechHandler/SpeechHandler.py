...
# Import role-specific speech generation modules
from . import common_speech
from . import werewolf_speech
from . import seer_speech
from . import witch_speech
from . import hunter_speech
...
class SpeechHandler:
    ...
    def reset_sheriff_speak_count(self):
        ...
        self.app.ui_handler.log_system("已重置警长发言计数")
    ...
    def player_speak(self, player_id):
        ...
        if not player.exists:
            self.app.ui_handler.log_system(f"玩家 {player_id} 不存在，无法发言。") # Changed to call ui_handler.log_system
            return
        ...
        if not is_last_words and self.app.state.phase == "night":
            if player.identity not in ["狼人", "预言家"]:
                self.app.ui_handler.log_system(f"玩家 {player_id} 在夜晚无法发言（身份：{player.identity}）。") # Changed to call ui_handler.log_system
                return

        # 准备阶段提示信息
        if is_last_words:
            self.app.ui_handler.log_system(f"玩家 {player_id} 准备进行遗言发言...") # Changed to call ui_handler.log_system
            
            # 如果是警长，添加警徽移交提示
            if player_id == self.app.state.sheriff_id:
                self.app.ui_handler.log_system(f"警长 {player_id} 可以在遗言中选择移交或销毁警徽") # Changed to call ui_handler.log_system
                
        elif self.app.state.phase == "day":
            # 白天发言准备提示
            identity_display = player.identity if player.identity != "空" else "玩家"
            if self.app.state.day == 0:
                self.app.ui_handler.log_system(f"{identity_display} {player_id} 准备进行警长竞选发言...") # Changed to call ui_handler.log_system
            else:
                self.app.ui_handler.log_system(f"{identity_display} {player_id} 准备进行白天发言...") # Changed to call ui_handler.log_system
        else:
            # 夜晚发言准备提示
            self.app.ui_handler.log_system(f"狼人 {player_id} 准备进行夜晚发言...") # Changed to call ui_handler.log_system
        ...
        if is_last_words:
            ...
            # 日志记录开始遗言
            self.app.ui_handler.log_system(f"玩家 {player_id} 开始遗言发言...") # Changed to call ui_handler.log_system
        ...
        # 如果不是遗言阶段，则按照正常的白天/夜晚阶段处理
            ...
            # 日志记录开始发言
            identity_display = player.identity if player.identity != "空" else "玩家"
            self.app.ui_handler.log_system(f"{identity_display} {player_id} 开始白天发言...") # Changed to call ui_handler.log_system

        # 1. 玩家不存在
        # 2. 活着的玩家在夜晚试图发言（除了狼人和预言家）
        if not player.exists:
            self.app.ui_handler.log_system(f"玩家 {player_id} 不存在，无法发言。")
            return
        ...
        if not is_last_words and self.app.state.phase == "night":
            if player.identity not in ["狼人", "预言家"]:
                self.app.ui_handler.log_system(f"玩家 {player_id} 在夜晚无法发言（身份：{player.identity}）。")
                return

        # Common information gathering (moved from various parts of the original function)
        ...
        header_footer_params = {
            ...
            "history_info_formatted": history_info_formatted
        }
        header = self._get_header(**header_footer_params)
        footer = self._get_footer(player_id=player_id, extra_info=extra_info)

        last_words_content = get_last_words_content() # Assuming this is a general utility
        common_prefix = "【注意】请务必将你的发言控制在300字以内，以免耽误其他玩家的发言时间。\n【重要】你正在玩狼人杀游戏，白天发言的所有内容会被所有玩家看到！不要在白天发言中透露不想让其他玩家知道的信息，即使用括号()或其他方式标注，这些内容也会被所有人看到！\n"
        common_suffix = "\n提醒】请保持发言简洁明了，限于300字以内。\n【再次强调】白天的所有发言都是公开的，所有玩家都能看到！不要试图使用括号或任何标记方式隐藏信息！"

        prompt_params = {
            "handler": self, # Pass self to allow role-specific functions to call helper methods
            "player": player,
            ...
            "get_sheriff_guidance_func": self._get_sheriff_guidance # Pass as a function
        }
        
        # Determine phase and set phase-specific parameters
        if is_last_words:
            prompt_params["phase_indicator"] = "【当前阶段：遗言发言阶段】\n"
            prompt_params["start_line"] = f"玩家 {player_id} 开始遗言...\n"
            self.app.ui_handler.log_system(f"玩家 {player_id} 准备进行遗言发言...")
            if player_id == self.app.state.sheriff_id:
                 self.app.ui_handler.log_system(f"警长 {player_id} 可以在遗言中选择移交或销毁警徽")

        elif self.app.state.phase == "day":
            identity_display = player.identity if player.identity != "空" else "玩家"
            if self.app.state.day == 0:
                prompt_params["phase_indicator"] = "【当前阶段：警长竞选发言阶段】\n"
                prompt_params["start_line"] = f"玩家 {player_id} 开始警长竞选发言...\n"
                self.app.ui_handler.log_system(f"{identity_display} {player_id} 准备进行警长竞选发言...")
            else:
                prompt_params["phase_indicator"] = "【当前阶段：白天发言阶段】\n"
                prompt_params["start_line"] = f"玩家 {player_id} 开始发言...\n"
                self.app.ui_handler.log_system(f"{identity_display} {player_id} 准备进行白天发言...")
        else: # Night phase (only werewolves and seers, but seer speech is handled by VoteHandler)
            prompt_params["phase_indicator"] = "【当前阶段：夜晚发言阶段】\n"
            prompt_params["start_line"] = f"玩家 {player_id} 开始夜间发言...\n"
            self.app.ui_handler.log_system(f"狼人 {player_id} 准备进行夜晚发言...")

        # Delegate prompt generation to role-specific modules
        prompt = ""
        if player.identity == "平民":
            prompt = common_speech.generate_common_speech_prompt(**prompt_params)
        elif player.identity == "狼人":
            prompt = werewolf_speech.generate_werewolf_speech_prompt(**prompt_params)
        elif player.identity == "预言家":
            prompt = seer_speech.generate_seer_speech_prompt(**prompt_params)
        elif player.identity == "猎人":
            prompt = hunter_speech.generate_hunter_speech_prompt(**prompt_params)
        elif player.identity == "女巫":
            prompt = witch_speech.generate_witch_speech_prompt(**prompt_params)
        else: # Fallback for "空" or any other unexpected identity
            self.app.ui_handler.log_system(f"玩家 {player_id} 身份为 {player.identity}，使用通用发言模板。")
            prompt = common_speech.generate_common_speech_prompt(**prompt_params)
        ...
        def speech_callback(final_answer_text):
            ...
            # Sheriff and Hunter logic from original callback needs to be here or called from here
            self._handle_sheriff_actions_in_speech(player_id, text_parsed, is_last_words)
            if player.identity == "猎人" and is_last_words:
                 self._handle_hunter_actions_in_speech(player_id, text_parsed)
            ...
            # 在游戏摘要中显示发言
            self.app.ui_handler.log_player(player_id, f"玩家 {player_id} 的发言：\n{text_parsed}\n")
            
            # 根据 TTS 开关状态播放语音
            if self.app.tts_enabled:
                play_tts(text_parsed, player_id, self.app.tts_speed)
        
        if not prompt: # If prompt wasn't generated (e.g. role not handled yet)
            self.app.ui_handler.log_system(f"错误：未能为玩家 {player_id} (身份: {player.identity}) 生成发言提示。")
            return
    ...
    def _get_sheriff_guidance(self, player_id): # Renamed from get_sheriff_guidance to avoid conflict if it was a global func
        sheriff_guidance = ""
        if self.app.state.sheriff_id == player_id:
            if player_id not in self.sheriff_speak_count:
                self.sheriff_speak_count[player_id] = 0
    ...
    def _handle_sheriff_actions_in_speech(self, player_id, text_parsed, is_last_words):
        if is_last_words and player_id == self.app.state.sheriff_id:
            ...
            if sheriff_transfer:
                ...
                if target_id in self.app.state.players and self.app.state.players[target_id].exists and self.app.state.players[target_id].alive:
                    ...
                    self.app.ui_handler.log_system(f"警长 {player_id} 将警徽传递给了玩家 {target_id}")
                    self.app.ui_handler.update_sheriff_labels()
                else:
                    self.app.ui_handler.log_system(f"警长 {player_id} 尝试将警徽传递给无效的玩家 {target_id}，警徽将被销毁")
                    self.app.state.sheriff_id = None # Explicitly destroy if target is invalid
                    self.app.ui_handler.update_sheriff_labels()
            
            sheriff_destroy = re.search(r'\[销毁警徽\]|【销毁警徽】|销毁警徽', text_parsed)
            if sheriff_destroy: # This will override transfer if both are present, or if no valid transfer
                self.app.state.sheriff_id = None
                self.app.ui_handler.log_system(f"警长 {player_id} 销毁了警徽")
                self.app.ui_handler.update_sheriff_labels()

    def _handle_hunter_actions_in_speech(self, player_id, text_parsed):
        # This logic was part of the original last_words_callback for hunters
        ...
        if hunter_shot:
            ...
            if target_player and target_player.exists and target_player.alive:
                ...
                self.app.ui_handler.log_system(f"猎人 {player_id} 开枪击杀了玩家 {target_player_id}！")
                ...
                self.app.ui_handler.log_system(f"[警告] 猎人 {player_id} 选择的开枪目标 {target_player_id} 不存在或已死亡。")
        else:
            self.app.ui_handler.log_system(f"猎人 {player_id} 没有选择开枪目标。")
                    ...
                    if match:
                        ...
                        if self.app.state.players[p_id].alive and self.app.state.players[p_id].exists:
                            ...
                            try:
                                ...
                                self.app.ui_handler.log_system(f"[警告] 读取玩家 {p_id} 白天发言失败: {e}") # Changed to call ui_handler.log_system
        return day_speeches
                    ...
                    if filename.startswith("玩家") and filename.endswith("白天发言.txt"):
                        ...
                        if match:
                            ...
                            try:
                                ...
                                self.app.ui_handler.log_system(f"[警告] 读取玩家 {p_id} 历史白天发言失败: {e}") # Changed to call ui_handler.log_system
                if day_speeches:
                    ...
                    if filename.startswith("玩家") and filename.endswith("白天投票.txt"):
                        ...
                        if match:
                            ...
                            try:
                                ...
                                self.app.ui_handler.log_system(f"[警告] 读取玩家 {p_id} 历史白天投票失败: {e}") # Changed to call ui_handler.log_system
                if day_votes:
                        ...
                        if match:
                            ...
                            if self.app.state.players[p_id].identity == "狼人":  # 仅读取狼人发言
                                ...
                                try:
                                    ...
                                    self.app.ui_handler.log_system(f"[警告] 读取玩家 {p_id} 历史夜晚发言失败: {e}") # Changed to call ui_handler.log_system
                if day_speeches:
                        ...
                        if match:
                            ...
                            if self.app.state.players[p_id].identity == "狼人":  # 仅读取狼人投票
                                ...
                                try:
                                    ...
                                    self.app.ui_handler.log_system(f"[警告] 读取玩家 {p_id} 历史夜晚投票失败: {e}") # Changed to call ui_handler.log_system
                if day_votes:
    ...
    def _read_check_record(self):
        ...
        if os.path.exists(check_file):
            try:
                ...
                self.app.ui_handler.log_system(f"[警告] 读取查验记录文件 record/查验.txt 失败: {e}") # Changed to call ui_handler.log_system
            ...
            if os.path.exists(day_folder):
                ...
                if os.path.exists(player_file):
                    try:
                        ...
                        self.app.ui_handler.log_system(f"[警告] 读取玩家 {player_id} 第{day}天白天投票失败: {e}") # Changed to call ui_handler.log_system
            ...
            if os.path.exists(day_folder):
                ...
                if os.path.exists(player_file):
                    try:
                        ...
                        self.app.ui_handler.log_system(f"[警告] 读取玩家 {player_id} 第{day}天夜晚投票失败: {e}") # Changed to call ui_handler.log_system
                    ...
                    if match:
                        ...
                        if self.app.state.players[p_id].alive and self.app.state.players[p_id].exists:
                            ...
                            try:
                                ...
                                self.app.ui_handler.log_system(f"[警告] 读取玩家 {p_id} 警长竞选发言失败: {e}") # Changed to call ui_handler.log_system
    ...
    def _read_player_sheriff_vote(self, player_id):
        ...
        try:
            ...
            self.app.ui_handler.log_system(f"读取警长投票记录时出错: {e}") # Changed to call ui_handler.log_system
            return "没有警长投票记录。\n"
    ...
    def _read_last_words(self):
        ...
        try:
            ...
            self.app.ui_handler.log_system(f"读取遗言记录时出错: {e}") # Changed to call ui_handler.log_system
            return "读取遗言记录时出错。\n"
    ...
    def prepare_last_words(self, player_id):
        ...
        if not player.exists or player.alive:
            self.app.ui_handler.log_system(f"[错误] 玩家 {player_id} 不存在或未死亡，无法发表遗言") # Changed to call ui_handler.log_system
            return
        ...
        self.app.ui_handler.log_system(f"玩家 {player_id} 开始发表遗言...") # Changed to call ui_handler.log_system
            ...
            if player.identity == "猎人":
                ...
                if hunter_shot:
                    ...
                    if target_player and target_player.exists and target_player.alive:
                        ...
                        self.app.ui_handler.log_system(f"[警告] 猎人 {player_id} 选择的开枪目标 {target_player_id} 不存在或已死亡。") # Changed to call ui_handler.log_system
                else:
                    self.app.ui_handler.log_system(f"猎人 {player_id} 没有选择开枪目标。") # Changed to call ui_handler.log_system
