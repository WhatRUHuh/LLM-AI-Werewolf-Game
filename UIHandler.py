...
class UIHandler:
    def __init__(self, root, app):
        ...
        self.night_theme = "cosmo"  # 夜晚主题 - 中等灰色主题，现代风格
    ...
    def create_widgets(self):
        ...
        ttk.Checkbutton(self.config_frame, text="启用 TTS", variable=self.app.tts_enabled_var, command=self.toggle_tts).grid(row=2, column=4, padx=5, pady=5, sticky="w")
        ...
        self.app.tts_speed_combo.bind("<<ComboboxSelected>>", lambda e: self.set_tts_speed(self.app.tts_speed_var.get()))
    ...
    def apply_theme(self, theme_name):
        ...
        if theme_name == self.night_theme:
            self.log_system("【主题切换】已切换至夜晚主题")
        else:
            self.log_system("【主题切换】已切换至白天主题")
    ...
    def create_player_frame(self, i):
        ...
        model_options = ["gemini", "deepseek", "glm4", "sparkmax", "cohere", "mistral", "qwq", "hunyuan", "Ollama"]
    ...
    def update_sheriff_labels(self):
        ...
        if sheriff_id is not None and sheriff_id in self.app.sheriff_labels:
            ...
            self.log_system(f"显示玩家 {sheriff_id} 的警长标签 {'(已死亡)' if not self.app.state.players[sheriff_id].alive else ''}")

    def toggle_tts(self):
        """切换 TTS 启用状态"""
        self.app.tts_enabled = not self.app.tts_enabled
        self.log_system(f"TTS 已 {'启用' if self.app.tts_enabled else '禁用'}")
        
    def set_tts_speed(self, speed):
        """设置 TTS 语音速度"""
        self.app.tts_speed = speed
        self.log_system(f"TTS 语音速度已设置为 {speed}")

    def log_system(self, message):
        """记录系统日志"""
        message = message.replace("\\n", "\n")
        self.app.summary_text.insert(tk.END, f"{message}\n", "system")
    ...
    def log_player(self, player_id, message):
        ...
        message = message.replace("\\n", "\n")
        tag = f"p{player_id}"
        if self.app.player_colors.get(player_id):
            self.app.summary_text.tag_configure(tag, foreground=self.app.player_colors.get(player_id))
        self.app.summary_text.insert(tk.END, f"{message}\n", tag)
        self.app.summary_text.see(tk.END)
